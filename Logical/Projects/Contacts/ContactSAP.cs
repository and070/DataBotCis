...
namespace DataBotV5.Logical.Projects.Contacts
{
    ...
    class ContactSAP
    {
        ...
        public string CreateContactSAP(ContactInfo contactInfo)
        {
            string resp = "";
            ...
            if (string.IsNullOrEmpty(contactInfo.respuesta))
            {
                ...
                try
                {
                    ...
                    telefonos.Clear();
                    foreach (phones phone in contactInfo.telefonos)
                    {
        ...
        public string UpdateContactSAP(ContactInfo contactInfo)
        {
        ...
        public ContactInfo ValData(ContactInfo contactInfo)
        {
            ...
            foreach (phones phone in contactInfo.telefonos)
            {
                ...
                {
                    if (phone.MOBILE.Substring(0, 1) == "(")
                    { phone.MOBILE = phone.MOBILE.Substring(5, phone.MOBILE.Length - 5); }

                    if ((phone.MOBILE.IndexOf("ext") + 1) > 0)
                    { phone.MOBILE = phone.MOBILE.Substring(0, phone.MOBILE.IndexOf("ext") - 1); }

                    phone.MOBILE = phone.MOBILE.Replace("+", "");
                    phone.MOBILE = val.EditPhone(phone.MOBILE);
                }


                if (!string.IsNullOrWhiteSpace(contactInfo.pais))
                {
                    int phoneDigits = val.GetExpectedPhoneDigitsByCountry(contactInfo.pais);


                    if (phoneDigits != phone.TELEPHONE.ToString().Length && phoneDigits > 0 && phone.TELEPHONE.ToString().Length > 0)
                    {
                        contactInfo.respuesta = er + $" El telefono {phone.TELEPHONE} no posee la cantidad de caracteres del país {phoneDigits}";
                        phone.TELEPHONE = "";
                    }

                    if (phoneDigits != phone.MOBILE.ToString().Length && phoneDigits > 0 && phone.MOBILE.ToString().Length > 0)
                    {
                        contactInfo.respuesta = er + $" El Celular {phone.MOBILE} no posee la cantidad de caracteres del país {phoneDigits}";
                        phone.MOBILE = "";
                    }
                ...
                if (phone.MOBILE != "" || phone.TELEPHONE != "")
                {
                    telefonos.Add(phone);
    ...
    class ContactInfo
    {
        ...
        public List<phones> telefonos { get; set; }
